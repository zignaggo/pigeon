  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div
          className={cn("text-foreground leading-tight font-extrabold", big ? "text-[28px]" : "text-[19px]")}
          style={{ letterSpacing: -0.6 }}
        >
          {title}
        </div>
        {subtitle && (
          <div className="text-muted-foreground mt-[3px] flex items-center gap-1.5 text-[12.5px]">{subtitle}</div>
        )}
      </div>
      {right}
    </div>
...
export function PMSectionLabel({ children, right }: { children: ReactNode; right?: ReactNode }) {
  return (
    <div className="flex items-center px-1.5 pb-2">
      <div className="text-muted-foreground flex-1 text-[11.5px] font-bold uppercase tracking-[0.8px]">{children}</div>
      {right && (
...
export function PMPeerRow({ peer, onTap, isLast }: { peer: Peer; onTap?: (p: Peer) => void; isLast?: boolean }) {
  return (
    <button
      type="button"
      onClick={() => onTap?.(peer)}
    ...
    >
      <div className="relative shrink-0">
        <PigeonAvatar name={peer.mono} tint={peer.tint} size={44} shape="squircle" />
      ...
      <div className="min-w-0 flex-1">
        <div className="text-foreground text-[15px] font-bold" style={{ letterSpacing: -0.2 }}>
          {peer.name}
        </div>
      ...
      <div className="flex shrink-0 flex-col items-end gap-[5px]">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-muted-foreground text-[11px]">{peer.distance}</span>
        </div>
        <svg width="7" height="12" viewBox="0 0 8 14" className="text-muted-foreground mr-px opacity-60">
...
export function PMFileRow({
  ...
  return (
    <div className={cn("flex items-center gap-3 px-[15px] py-3", !isLast && "border-border border-b")}>
      <FileIcon ext={file.ext} size={34} />
      <div className="min-w-0 flex-1">
        <div className="text-foreground truncate text-[13.5px] font-semibold">{file.name}</div>
        <div className="text-muted-foreground mt-0.5 flex gap-2 text-[11px]" style={{ fontFamily: MONO }}>
          <span>{file.size}</span>
          {file.badge && <span>· {file.badge}</span>}
          {state === "sending" && <span className="text-primary">· enviando…</span>}
          {state === "done" && <span style={{ color: "var(--chart-3)" }}>· enviado</span>}
        </div>
        {progress != null && (
          <div className="bg-muted mt-[7px] h-1 overflow-hidden rounded-sm">
            <div
              className="h-full rounded-sm transition-[width] duration-300"
              style={{ width: `${progress}%`, background: state === "done" ? "var(--chart-3)" : "var(--primary)" }}
            />
          </div>
        )}
      </div>
...
const ICONS: Record<Tab, (active: boolean) => ReactNode> = {
  rede: (a) => (
    <svg width="22" height="22" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth={a ? 1.9 : 1.6} strokeLinecap="round" strokeLinejoin="round">
...
export function PMTabBar({ active = "rede", onChange }: { active?: Tab; onChange?: (t: Tab) => void }) {
  return (
      ...
      {TABS.map((t) => {
        ...
        return (
          <button
            ...
            className={cn("flex flex-1 flex-col items-center gap-1", on ? "text-primary" : "text-muted-foreground")}
          >
            {ICONS[t.id](on)}
            <span className={cn("text-[10.5px]", on ? "font-bold" : "font-medium")}>{t.label}</span>
          </button>
        );
      })}
    </div>
...
// ── Compact radar hero ───────────────────────────────────────
export function PMRadarHero({ count = 5, network = "Wi-Fi Casa", me = "EU" }: { count?: number; network?: string; me?: string }) {
  ...
  return (
    <div className="relative h-[200px] shrink-0 overflow-hidden">
      ...
      <div
        ...
        style={{ background: "radial-gradient(circle, color-mix(in oklab, var(--primary) 22%, transparent), transparent 62%)" }}
      />
      {/* static rings */}
      {[58, 92, 128].map((r) => (
        <div
      ...
      {[0, 1.1].map((d) => (
        <div
          ...
          style={{ marginLeft: -128, marginTop: -128, borderColor: "var(--primary)", animation: `pmPulse 3.4s ease-out ${d}s infinite` }}
        />
      ))}
      {/* peer dots */}
      ...
      <div className="absolute left-1/2 top-[54%] -translate-x-1/2 -translate-y-1/2">
        <div
          ...
          style={{ boxShadow: "0 10px 26px color-mix(in oklab, var(--primary) 45%, transparent), 0 0 0 7px var(--accent)", animation: "pmFloat 4s ease-in-out infinite" }}
        >
          <span className="text-[8px] uppercase tracking-[1px] opacity-85">Você</span>
      ...
      <div className="bg-card border-border text-muted-foreground absolute bottom-2.5 left-1/2 flex -translate-x-1/2 items-center gap-[7px] whitespace-nowrap rounded-2xl border px-3 py-1.5 text-[11.5px] shadow-[0_4px_12px_rgba(0,0,0,0.25)]">
        <svg width="13" height="13" viewBox="0 0 12 12" fill="none" stroke="var(--primary)" strokeWidth="1.5" strokeLinecap="round">
...
export function PushHeader({ onBack, eyebrow, title }: { onBack?: () => void; eyebrow?: string; title: ReactNode }) {
  return (
    <div
      className="bg-card/80 border-border flex shrink-0 items-center gap-2 border-b pl-3 pr-4 pb-3 backdrop-blur-md"
      style={{ paddingTop: 22 }}
    >
      <button type="button" onClick={onBack} aria-label="Voltar" className="text-primary flex size-10 shrink-0 items-center justify-center rounded-[13px]">
        <svg width="12" height="20" viewBox="0 0 12 20" fill="none">
          <path d="M10 2L2 10l8 8" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      </button>
      <div className="min-w-0 flex-1">
        {eyebrow && <div className="text-muted-foreground text-[11px] font-bold uppercase tracking-[0.6px]">{eyebrow}</div>}
        <div className="text-foreground mt-px flex items-center gap-2 text-[18px] font-extrabold" style={{ letterSpacing: -0.4 }}>
          {title}
        </div>
      </div>
    </div>
...
export function BottomBar({ children }: { children: ReactNode }) {
  return (
    <div className="bg-card/80 border-border flex shrink-0 gap-3 border-t px-4 pb-7 pt-3 backdrop-blur-md">{children}</div>
  );
}

// ── Progress ring ────────────────────────────────────────────
export function RingBig({ pct, done, size = 96 }: { pct: number; done?: boolean; size?: number }) {
  const r = size / 2 - 7;
  const c = 2 * Math.PI * r;
  ...
  return (
    <div className="relative mx-auto" style={{ width: size, height: size }}>
      <svg width={size} height={size} style={{ transform: "rotate(-90deg)" }}>
        <circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke="var(--muted)" strokeWidth="7" />
        <circle
          cx={size / 2}
          cy={size / 2}
          r={r}
      ...
      <div className="absolute inset-0 flex items-center justify-center" style={{ color: stroke }}>
        {done ? (
          <svg width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" style={{ animation: "pmRise .35s ease" }}>
            <path d="M5 12.5l4.5 4.5L19 7.5" />
          </svg>
        ) : (
          <svg width="30" height="30" viewBox="0 0 22 22" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" style={{ animation: "pmFloat 2.5s ease-in-out infinite" }}>
