...
import { PushHeader, BottomBar, PMSectionLabel, PMCard, PMFileRow, RingBig } from "@/components/pigeon/mobile";
...
function TransferScreen() {
  ...
  const fileState = (i: number): { state: "queued" | "sending" | "done"; progress: number } => {
    ...
    >
      ...
      <div className="pm-screen flex-1 overflow-auto p-4">
        <PMCard className="px-5 py-[22px] text-center">
          ...
          <div className="text-foreground mt-3.5 text-[30px] font-extrabold" style={{ letterSpacing: -1 }}>
            {done ? "Enviado!" : `${Math.round(pct)}%`}
          </div>
          <div className="text-muted-foreground mt-0.5 text-[13px]" style={{ fontFamily: MONO }}>
            {done ? `${TOTAL_SIZE} · 4 arquivos entregues` : `${mbSent} MB / ${TOTAL_SIZE}`}
          </div>
          {!done && (
            <div className="text-muted-foreground mt-3 flex flex-wrap justify-center gap-4 text-xs" style={{ fontFamily: MONO }}>
              <span>↑ 24,6 MB/s</span>
              <span>~{Math.max(1, Math.round((100 - pct) / 8))} s</span>
              <span className="inline-flex items-center gap-1" style={{ color: "var(--chart-3)" }}>
          ...
          <div className="bg-muted mt-4 h-[7px] overflow-hidden rounded">
            <div
              ...
              style={{
                width: `${pct}%`,
                background: done ? "var(--chart-3)" : "linear-gradient(90deg, var(--primary), var(--chart-2))",
        ...
        <div className="mt-[18px]">
          ...
          <PMCard>
            {MFILES.map((f, i) => {
              ...
              return <PMFileRow key={f.name} file={f} progress={fs.progress} state={fs.state} isLast={i === MFILES.length - 1} />;
            })}
          </PMCard>
        </div>
      </div>
